...
import { FaEye, FaEyeSlash, FaEnvelope, FaLock, FaUser, FaImage, FaGoogle } from "react-icons/fa";
...
export default function RegisterClient() {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { name, email, profileUrl, password, confirmPassword } = data as Record<string, string>;
    ...
    if (!hasNumber) {
      toast.error("Access Key rejection: Must contain at least one digit matrix [0-9].");
      return;
    }
    if (!hasSpecialChar) {
    ...
    // Validation sequence cleared successfully
    console.log("Submitting cryptographic data bundle:", {
      name,
      email,
      profileUrl,
      password,
    ...
    try {
      ...
      toast.error("Node initialization sequence error.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        ...
        >
          ...
          <div className="w-full md:w-1/2 relative flex items-center justify-center p-8 lg:p-12 overflow-hidden min-h-[250px] md:min-h-full">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t md:bg-gradient-to-r from-[#0d0f1a] via-[#0d0f1a]/95 to-[#0d0f1a]/40" />
            ...
            <div className={`relative z-10 space-y-3 max-w-sm mr-auto transition-all duration-[1000ms] delay-300 ease-out ${
              ...
              <h1 className="text-2xl lg:text-3xl font-black font-mono tracking-wider text-white uppercase leading-none">
                Establish Your <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 drop-shadow-[0_0_15px_rgba(99,102,241,0.3)]">Network Node</span>
              </h1>
              <p className="text-[11px] font-mono text-gray-300 leading-relaxed tracking-wide">
                Join the STASH cooperative network ecosystem. Initialize a decentralized proxy account to collect drop assets, customize parameters, and secure your inventories.
              </p>
              <div className="pt-2 border-t border-white/5 flex gap-4 text-[9px] font-mono text-gray-500 uppercase tracking-widest">
                <div>Security: <span className="text-gray-400">AES_256</span></div>
                <div>Status: <span className="text-indigo-400 animate-pulse">Awaiting Sync</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-center p-6 lg:p-10 bg-[#090b14]/50 border-t md:border-t-0 md:border-l border-white/[0.04]">
            ...
            <div className={`space-y-4 transition-all duration-[1000ms] delay-500 ease-out ${
              ...
              <form onSubmit={handleRegister} className="space-y-3">
                ...
                {/* Alias Identity & Identity Email */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1 group/input">
                    <label className="text-[9px] font-mono font-bold tracking-widest text-gray-400 group-focus-within/input:text-indigo-400 uppercase pl-1">Alias Identity</label>
                    <div className="relative flex items-center bg-[#05060c] border border-white/10 focus-within:border-indigo-500/60 rounded-lg px-3 h-9 transition-all duration-300">
                      ...
                      <input type="text" name="name" required placeholder="Operator..." className="w-full bg-transparent text-xs font-mono focus:outline-none text-white placeholder-gray-600 tracking-wide" />
                    </div>
                  </div>

                  <div className="space-y-1 group/input">
                    <label className="text-[9px] font-mono font-bold tracking-widest text-gray-400 group-focus-within/input:text-indigo-400 uppercase pl-1">Identity Email</label>
                    <div className="relative flex items-center bg-[#05060c] border border-white/10 focus-within:border-indigo-500/60 rounded-lg px-3 h-9 transition-all duration-300">
                      <FaEnvelope className="text-gray-500 text-[10px] mr-2 shrink-0 group-focus-within/input:text-indigo-400" />
                      <input type="email" name="email" required placeholder="Enter link..." className="w-full bg-transparent text-xs font-mono focus:outline-none text-white placeholder-gray-600 tracking-wide" />
                    </div>
                  </div>
                </div>
                ...
                {/* Profile Image URL */}
                <div className="space-y-1 group/input">
                  <label className="text-[9px] font-mono font-bold tracking-widest text-gray-400 group-focus-within/input:text-indigo-400 uppercase pl-1">Profile Image URL</label>
                  <div className="relative flex items-center bg-[#05060c] border border-white/10 focus-within:border-indigo-500/60 rounded-lg px-3 h-9 transition-all duration-300">
                    <FaImage className="text-gray-500 text-[10px] mr-2 shrink-0 group-focus-within/input:text-indigo-400" />
                    <input type="url" name="profileUrl" required placeholder="https://assets.stash.gg/avatar.png" className="w-full bg-transparent text-xs font-mono focus:outline-none text-white placeholder-gray-600 tracking-wide" />
                  </div>
                </div>

                {/* Password Fields Split Matrix */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1 group/input">
                    <label className="text-[9px] font-mono font-bold tracking-widest text-gray-400 group-focus-within/input:text-indigo-400 uppercase pl-1">Access Key</label>
                    <div className="relative flex items-center bg-[#05060c] border border-white/10 focus-within:border-indigo-500/60 rounded-lg px-3 h-9 transition-all duration-300">
                      ...
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-gray-500 hover:text-white transition-colors ml-1 focus:outline-none shrink-0">
                        {showPassword ? <FaEyeSlash className="text-xs cursor-pointer" /> : <FaEye className="text-xs" />}
                      </button>
                    </div>
                  </div>

                  <div className="space-y-1 group/input">
                    <label className="text-[9px] font-mono font-bold tracking-widest text-gray-400 group-focus-within/input:text-indigo-400 uppercase pl-1">Confirm Key</label>
                    <div className="relative flex items-center bg-[#05060c] border border-white/10 focus-within:border-indigo-500/60 rounded-lg px-3 h-9 transition-all duration-300">
                      <FaLock className="text-gray-500 text-[10px] mr-2 shrink-0 group-focus-within/input:text-indigo-400" />
                      <input type={showConfirmPassword ? "text" : "password"} name="confirmPassword" required placeholder="••••••••" className="w-full bg-transparent text-xs font-mono focus:outline-none text-white placeholder-gray-600 tracking-widest" />
                      <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="text-gray-500 hover:text-white transition-colors ml-1 focus:outline-none shrink-0">
                        {showConfirmPassword ? <FaEyeSlash className="text-xs cursor-pointer" /> : <FaEye className="text-xs" />}
                      </button>
                    </div>
                  </div>
                </div>
              ...
              <div className="text-center pt-2 border-t border-white/[0.04]">
                <p className="text-[10px] font-mono text-gray-500 uppercase tracking-widest">
                  Already registered?{" "}
                  <Link href="/auth/login" className="text-indigo-400 hover:text-white font-bold ml-1 transition-colors underline-offset-4 hover:underline">
                    Access Portal
                  </Link>
                </p>
              </div>
